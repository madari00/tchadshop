...
if (isset($_GET['delete'])) {
    if ($_GET['delete'] === 'all' && isset($_GET['client_id'])) {
        $client_id = intval($_GET['client_id']);
        $conn->query("DELETE FROM messages_clients WHERE client_id = $client_id");
        header("Location: message_client.php?client_id=$client_id");
        exit();
    } elseif (isset($_GET['message_id'])) {
        $message_id = intval($_GET['message_id']);
        $conn->query("DELETE FROM messages_clients WHERE id = $message_id");
        
        if (isset($_GET['client_id'])) {
            header("Location: message_client.php?client_id=" . intval($_GET['client_id']));
        } else {
...
$clients_sql = "SELECT 
            ...
            cl.email,
            cl.invite,
            MAX(mc.created_at) AS dernier_message,
            COUNT(mc.id) AS total_messages,
            SUM(CASE WHEN mc.lu = 0 THEN 1 ELSE 0 END) AS non_lus
        FROM clients cl
        LEFT JOIN messages_clients mc ON cl.id = mc.client_id
        GROUP BY cl.id, cl.nom, cl.email, cl.invite
        ORDER BY dernier_message DESC";

$clients_result = $conn->query($clients_sql);

// 🔎 Récupérer les messages du client sélectionné
if ($client_id && is_numeric($client_id)) {
    // Marquer tous les messages comme lus
    $update_stmt = $conn->prepare("UPDATE messages_clients SET lu = 1 WHERE client_id = ?");
    ...
    $client_stmt->execute();
    $current_client = $client_stmt->get_result()->fetch_assoc();
    $client_stmt->close();
    
    // Récupérer les messages
    $messages_sql = "SELECT * FROM messages_clients 
                    WHERE client_id = ? 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $client_id) {
    ...
    // Gestion de l'audio - méthode corrigée
    if (!empty($_FILES['audio_file']['tmp_name'])) {
        ...
        if (in_array(strtolower($file_extension), $allowed_extensions)) {
            ...
            $target_dir = __DIR__ . "/../client/uploads/audio_support/";
    ...
    if (!empty($reponse) || $audio_path) {
        $stmt = $conn->prepare("INSERT INTO messages_clients (client_id, sujet, message, audio_path, repondu) 
                               VALUES (?, 'Réponse admin', ?, ?, 1)");
        $stmt->bind_param("iss", $client_id, $reponse, $audio_path);
        if ($stmt->execute()) {
...
<head>
    ...
    <title>📥 Conversations Clients - TchadShop Admin</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* Styles optimisés et corrigés */
        :root {
            --primary: #2c6e49;
        ...
        <div class="conversations-list">
            <?php if ($clients_result->num_rows > 0): ?>
                <?php while ($row = $clients_result->fetch_assoc()): 
                    $is_active = $client_id == $row['client_id'];
                    ?>
                    <a href="?client_id=<?= $row['client_id'] ?>" 
                       class="conversation-item <?= $is_active ? 'active' : '' ?>">
                        ...
                        <div class="conversation-info">
                            <div class="client-name">
                                ...
                                <?php if ($row['invite']): ?>
                                    <span class="invite-badge">Invité</span>
                                <?php endif; ?>
                            </div>
            ...
            <?php else: ?>
                <div class="empty-list">
                    ...
                    <p>Aucun client n'a encore envoyé de message</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="chat-container">
        <?php if ($current_client): ?>
            <div class="chat-header">
                <div class="chat-avatar">
                    <?= strtoupper(substr($current_client['nom'], 0, 1)) ?>
                </div>
                <div class="chat-info">
                    <div class="chat-client-name">
                        <?= htmlspecialchars($current_client['nom']) ?>
                        <?php if ($current_client['invite']): ?>
                            <span class="invite-badge">Invité</span>
                        <?php endif; ?>
                    </div>
                    <div class="chat-client-email">
                        <?= htmlspecialchars($current_client['email'] ?? '') ?>
                    </div>
                </div>
            ...
            <div class="chat-messages">
                <?php if (count($messages) > 0): ?>
                    <?php foreach ($messages as $message): 
                        $is_admin = !empty($message['reponse']) || $message['sujet'] === 'Réponse admin';
                        ?>
                        <div class="message <?= $is_admin ? 'admin-message' : 'client-message' ?>">
                            ...
                            <div class="message-content">
                                <?php if (!empty($message['message'])): ?>
                                    <?= nl2br(htmlspecialchars($is_admin ? $message['message'] : $message['message'])) ?>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($message['audio_path'])): ?>
                                    <div class="audio-container">
                                        ...
                                        <audio controls>
                                            <source src="../client/<?= htmlspecialchars($message['audio_path']) ?>" type="audio/mpeg">
                                            <source src="../client/<?= htmlspecialchars($message['audio_path']) ?>" type="audio/webm">
                                            <source src="../client/<?= htmlspecialchars($message['audio_path']) ?>" type="audio/wav">
                                            <source src="../client/<?= htmlspecialchars($message['audio_path']) ?>" type="audio/ogg">
                ...
                <?php else: ?>
                    <div class="no-conversation">
                        ...
                        <p>Ce client n'a pas encore envoyé de message</p>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <?php else: ?>
            <div class="no-conversation">
                ...
                <p>Sélectionnez un client dans la liste pour afficher la conversation</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Enregistrement vocal - méthode corrigée
    ...
    confirmDelete.addEventListener('click', function() {
        ...
        if (currentMessageToDelete) {
            ...
            window.location.href = `?delete=message&message_id=${currentMessageToDelete}&client_id=<?= $client_id ?>`;
        } else {
            // Supprimer toute la conversation
            window.location.href = `?delete=all&client_id=<?= $client_id ?>`;
        }
    });
